...
## [Unreleased]
### Added
- Add JIT (`torch.compile`) auto-detection in `ptyrad/runtime/jit.py`. `recon_params.compiler_configs.enable` now defaults to `'auto'`, which checks right before the reconstruction loop whether JIT compilation is achievable on the current machine (PyTorch version, TorchDynamo/TorchInductor support, compute device, CUDA compute capability, Triton on GPU / C++ compiler on CPU) and then compiles a tiny function as a smoke test to functionally verify the toolchain. JIT is used when the check passes, otherwise PtyRAD logs the reason and gracefully falls back to eager mode. macOS is excluded from `'auto'` because Triton/TorchInductor is less stable there and can occasionally return incorrect numerics, so Mac users opt in with `enable: true`. `enable: true` and `enable: false` keep forcing JIT on/off, and the new `compiler_configs.auto_smoke_test` flag can skip the smoke-test warmup
...
- Add `pos_affine` constraint that fits the probe positions with a 2D affine model (scale, asymmetry, rotation, shear, plus a global translation) relative to the initial positions and pulls them back towards that fit. This suppresses noisy local position updates while letting a global scan miscalibration (e.g., rotation) be corrected as a whole. The `relax` param follows the same convention as the other constraints, so `relax: 0` is a hard constraint and `relax: 1` is a no-op. The fitted (scale, asymmetry, rotation, shear) are recorded in `convergence_iters` of the output `model.hdf5`. The constraint is automatically skipped for line scans, and only the positions selected by `INDICES_MODE` are fitted
- Add `PtychoModel.set_active_indices` so constraints that pool information across scan positions know which positions `INDICES_MODE` actually optimizes
### Changed
- Treat a constraint that is missing from `constraint_params` as disabled instead of raising a `KeyError`, so params files written before a new constraint was introduced keep working with `validate: false`
