...
def print_nested_dict(d, indent=0, verbose=True, leaf_inline_threshold=6):
    indent_str = "    " * indent
    for key, value in d.items():
        if isinstance(value, dict):
            ...
            if is_flat_leaf and len(value) <= leaf_inline_threshold:  # Determine whether to print inline or not
                ...
                print(f"{indent_str}{key}: {{{flat}}}", verbose=verbose)
            else:
                print(f"{indent_str}{key}:", verbose=verbose)
                print_nested_dict(value, indent + 1, verbose=verbose)
        elif isinstance(value, list) and all(not isinstance(i, (dict, list)) for i in value):
            print(f"{indent_str}{key}: {value}", verbose=verbose)
        else:
            print(f"{indent_str}{key}: {repr(value)}", verbose=verbose)