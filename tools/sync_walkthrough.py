...
SOURCE_WALKTHROUGH = Path("src/ptyrad/starter/params/walkthrough/")
DEST_DOCS_WALKTHROUGH = Path("docs/walkthrough")
...
def main():
    if not SOURCE_WALKTHROUGH.exists():
        print(f"❌ Walkthrough source not found: {SOURCE_WALKTHROUGH}")
        return

    print(f"🚀 Syncing Walkthroughs from '{SOURCE_WALKTHROUGH}'...")
    
    # Ensure destination exists
    DEST_DOCS_WALKTHROUGH.mkdir(parents=True, exist_ok=True)

    # Copy all YAML files
    for yaml_path in SOURCE_WALKTHROUGH.glob("*.yaml"):
        dest_path = DEST_DOCS_WALKTHROUGH / yaml_path.name
        ...
        md_path = DEST_DOCS_WALKTHROUGH / f"{yaml_path.stem}.md"
